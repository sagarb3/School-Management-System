...
//$inse= mysqli_query($conn,"INSERT INTO  studentmark (STUDENT_ID)VALUES ('$id')");
$insert= "INSERT INTO  student VALUES ('','$fname','$sex', '$age', '$dis','$pname', '$dbstudent', '$form','Active',curdate())";
mysqli_query($conn,"update table student set status='Active' where offering='day'");
...
?>
          ...
          <table>
            <tr>
              ...
              <td><select name="owner" required >
                  ...
                  <option>
                  <?php
						$result = mysqli_query($conn,"SELECT  STNAME FROM STUDENT WHERE STNAME NOT IN (SELECT image_id FROM image)");
						while($row = mysqli_fetch_array($result))
							{  
								echo '<option value="'.$row['STNAME'].'">';
								echo $row['STNAME'];
 ...
 if (isset($_POST['submit'])) {
								...
								$owner=$_POST['owner'];
                                mysqli_query($conn,"insert into image (image_id,location)
...
$do=mysqli_query($conn,"SELECT * from student , image where image.image_id='$search' AND student.STNAME='$search'")or die(mysqli_error());
