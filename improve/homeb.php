            ...
            <table  border="0" cellspacing="3" cellpadding="5"  id='mytable'summary="registering student">
              ...
              <tr>
                ...
                <td ><select name="firstname" required >
                    ...
                    <option>
                    <?php
						$result = mysqli_query($conn,"SELECT STNAME FROM student WHERE STNAME NOT IN (SELECT STNAME FROM payments) and STNAME NOT IN  (SELECT NAME FROM bursarystudent)") ;
						while($row = mysqli_fetch_array($result))
							{  
								echo '<option value="'.$row['STNAME'].'">';
								echo $row['STNAME'];
...
$ins= mysqli_query($conn,"INSERT INTO  itempay VALUES ('','$fnames','$item',$amount,'$date')") or die ('Some little problem: Amount should be int, tr again');
...
$result = mysqli_query($conn,"SELECT *FROM student WHERE STNAME NOT IN (SELECT STNAME FROM payments) and STNAME NOT IN  (SELECT NAME FROM bursarystudent)") ;							while($row = mysqli_fetch_array($result))
							{  
...
$famount=mysqli_query($conn,"select TERM, YEAR, TUTION, SUM(AMOUNT) AS tamount, SUM(DUES) AS tdues FROM payments GROUP BY YEAR , TERM ORDER BY YEAR DESC ");
echo '<table style="width:100%">';
...
while($fetch=mysqli_fetch_array($famount)){
...
$oamount=mysqli_query($conn,"select SUM(AMOUNT) AS tamount FROM itempay ");
$fetch=mysqli_fetch_array($oamount);
...
?>
    ...
    <div id="school_footer">
      <div align=center> Copyright &copy;<?php date('Y');?>Smogate </div>
    </div>
