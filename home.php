...
<div id="school_body_wrapper">
  <div id="school_wrapper">
    ...
    <div id="school_main">
      ...
      <?php
			...
			$qual=$_POST['quality'];
			$salary=$_POST['salary'];
			$insert= mysqli_query($conn,"INSERT INTO  teachersalary (TEACH_ID, SALARY, CREDIT)VALUES ('$fnames', '$salary','$salary')");
			$ins= mysqli_query($conn,"INSERT INTO  teacher VALUES ('','$fnames','$sexs','$ages','$qual','Active')");
    ...
    <?php
			...
			$salary=$_POST['salary'];
			$insert= mysqli_query($conn,"INSERT INTO nonstaffpay (NON_ID, SALARY, CREDIT)VALUES ('$fnames', '$salary','$salary')");
			$ins= mysqli_query($conn,"INSERT INTO  nonstaff VALUES ('','$fnames','$sexs','$ages','$dut','Active')");
    ...
    <?php 
			$male=mysqli_query($conn,"select *from student where SEX='M'");
			$countMALE=mysqli_num_rows($male);
			
			$female=mysqli_query($conn,"select *from student where SEX='F'");
			$countfemale=mysqli_num_rows($female);
  ...
  <?php
			$sel=mysqli_query($conn,"SELECT * FROM teacher");
			echo '<table style="width:100%;">';
...
<?php
			$sel=mysqli_query($conn,"SELECT * FROM nonstaff");
			echo '<table style="width:100%;">';
...
<?php
 ...
 where student.STNAME='$name' AND studentmark.student_id='$name' AND studentmark.YEAR='$ya' AND studentmark.TERM='$term' AND subject.code=studentmark.code AND student.STNAME=studentmark.student_id");
			 //$hy=mysqli_query("SELECT SUM(TEST) AS 'totaltest', SUM(EXAM) AS 'totalexam', AVG(TEST) AS 'avgtest', AVG(EXAM) AS 'avgexam' FROM studentmark  where FNAME='$name' AND student.STUDENT_ID='$snum' ");
...
<?php
			$sel=mysqli_query($conn,"SELECT * FROM teachercheck");
			echo '<table style="width:100%;">';
			...
			$selt=mysqli_query($conn,"SELECT COUNT(NAME) as times , NAME FROM teachercheck GROUP BY NAME") or die (mysqli_error()) ;
			while($fetch=mysqli_fetch_array($selt)){
...
<?php
			...
			$name=$_POST['bname'];
			$amount=$_POST['amount'];
			$getst=mysqli_query($conn,"select *from student where STNAME='$name' ");
			$fe=mysqli_fetch_array($getst);
			$class= $fe['CLASS'];
			$sex= $fe['SEX'];
			
			$goburs=mysqli_query($conn,"insert into bursarystudent values('','$name','$class','$sex','$amount')");
...
<?php
			$sel=mysqli_query($conn,"SELECT * FROM bursarystudent");
			echo '<table style="width:100%">';
  ...
  <table  border="0" cellspacing="3" cellpadding="5"  id='mytable'summary="registering student">
    ...
    <tr>
      ...
      <td ><select name="firstname" required >
          ...
          <option>
          <?php
									$result = mysqli_query($conn,"SELECT STNAME FROM student WHERE STNAME NOT IN (SELECT STNAME FROM payments)") ;
									while($row = mysqli_fetch_array($result))
										{  
											echo '<option value="'.$row['STNAME'].'">';
											echo $row['STNAME'];
...
<?php
			...
			$ins= mysqli_query($conn,"INSERT INTO  payments VALUES ('','$fnames','$class','$term',$year,$tution,$amount,$dues,$balance,'$date')") or die (mysqli_error());
...
<?php
			...
			$result = mysqli_query($conn,"SELECT *FROM student WHERE STNAME NOT IN (SELECT STNAME FROM payments) and STNAME NOT IN  (SELECT NAME FROM bursarystudent)") ;						
			while($row = mysqli_fetch_array($result))
										{  
...
<?php
			...
			$ins= mysqli_query($conn,"INSERT INTO  itempay VALUES ('','$fnames','$item',$amount,'$date')") or die (mysqli_error());
...
<?php
			$sel=mysqli_query($conn,"SELECT * FROM itempay");
			echo '<table style="width:100%">';
...
<?php
			$sel=mysqli_query($conn,"SELECT * FROM payments ORDER BY DATE DESC");
			echo '<table style="width:100%">';
...
<?php
			...
			$cm=$_POST['chairman'];
			$pat=$_POST['patron'];
			$insert=mysqli_query($conn,"insert into club values('$ccode','$name','$desc','$cm','$pat',curdate(), 'Active')");
			if($insert){
...
<?php
			...
			$selc=mysqli_query($conn,"select *from clubmember where CLUB_ID='$ccode' and MEMBER_ID='$stid'");
			$count=mysqli_num_rows($selc);
			if($count >0){
			echo 'Sorry! Student is already a registered member of club';
			 echo '<meta content="3;home.php?action=member" http-equiv="refresh" />';
			
			}else{
			$insert=mysqli_query($conn,"insert into clubmember values('$ccode', '$stid', curdate())")or die (mysqli_error());
			if($insert){
...
<form id="mytable" method="post">
  ...
  <select name="tid" required >
    ...
    <option>
    <?php
									$result = mysqli_query($conn,"SELECT  * FROM teacher, teachersalary where teacher.NAMES=teachersalary.TEACH_ID");
									while($row = mysqli_fetch_array($result))
										{  
...
<?php
			...
			$sel=mysqli_query($conn,"SELECT * FROM teacher, teachersalary WHERE teacher.NAMES='$id' AND teachersalary.TEACH_ID='$id' ");
...
<?php
			...
			$insert= mysqli_query($conn,"UPDATE teachersalary set PAYAMOUNT=$paid,CREDIT=$credit1,DAYPAY1='$day1',DAYPAY2='$day2'
...
<form id="mytable" method="post">
  ...
  <select name="tid" required >
    ...
    <option>
    <?php
									$result = mysqli_query($conn,"SELECT  * FROM nonstaff, nonstaffpay where nonstaff.NAME=nonstaffpay.NON_ID");
									while($row = mysqli_fetch_array($result))
										{  
...
<?php
			...
			$sel=mysqli_query($conn,"SELECT * FROM nonstaff, nonstaffpay WHERE nonstaff.NAME='$id' AND nonstaffpay.NON_ID='$id' ");
...
<?php
			...
			$insert= mysqli_query($conn,"UPDATE nonstaffpay set PAYAMOUNT=$paid,CREDIT=$credit1,DAYPAY1='$day1',DAYPAY2='$day2'
...
<?php
			$sel=mysqli_query($conn,"select *from club ORDER BY DATE DESC ");
			echo '<table style="width:100%">';
...
<?php
			$sel=mysqli_query($conn,"select *from club, student, clubmember where club.CLUB_ID=clubmember.CLUB_ID AND clubmember.MEMBER_ID=student.STNAME ORDER BY clubmember.DATE DESC");
			echo '<table style="width:100%">';
...
<table style="border: 3px double #FF6600; background:url(images/powder.gif);background-size:cover; padding:12px; color:#003366">
  <tr>
    <td scope="col" colspan="3" align="center">Smogate </td>
  </tr>
  <tr>
    <td colspan="3">&nbsp;</td>
  </tr>
  ...
  <tr>
    <td colspan="3" align="center">Pika Pika</td>
  </tr>
</table>
<?php
			}
...
<div id="school_footer">
        <div align=center> Copyright &copy; 2015 Smogate. Designed by Sagar </div>
