...
if (isset($_POST["search"]) && ($_POST["search"] != "")) {
	...
	$conn = mysqli_connect("localhost", $_POST["username"], $_POST["password"]) or die ("Error connecting to mysqli");
	mysqli_select_db($_POST["database"]);
	...
	for ($i = 0; $i < sizeof($aryTables); $i = $i + 1) {
		...
		$result = mysqli_query($conn,$sql);
		 ...
		 if (mysqli_num_rows($result)>0) {
			...
			echo "<br/><hr/><p>" .mysqli_num_rows($result).'  search results for  '.$_POST["search"].' in table '. $aryTables[$i] . "</p><br/>"; 
			...
			while ($aryData = mysqli_fetch_assoc($result)) {
			if ($flag%2 ==0 ) :
